...
if __name__ == "__main__":
    #ann_path = "scienceie/S0010938X1500195X.ann"
    ...
    anns = glob.glob("nlm"+"/*.key")
    for ann_path in anns:

        name = ann_path.split('.')[0]
        name = name.split('/')[1]
        gt = read_ann(ann_path)
        my_path = "nlm_res/" + name + "my.txt"
        my = read_file(my_path)
        rake_path = "nlm_res/" + name + "rake.txt"
        rake = read_file(rake_path)
        textrank_path = "nlm_res/" + name + "textrank.txt"
        textrank = read_file(textrank_path)
        tfidf_path = "nlm_res/" + name + "tfidf.txt"
