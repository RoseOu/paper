...
if __name__ == "__main__":
    ...
    input_file = glob.glob("se2010"+"/*.txt")
    for txt_path in input_file:
        print(txt_path)
        with open(txt_path,'r') as file:
            data = file.read()
    ...
    #-----------------总分值计算---------------------------------------
        name = txt_path.split('.')[0]
        name = name.split('/')[1]
        total_path = "se2010_res/" + name + "my.txt"
        tfidf_path = "se2010_res/" + name + "tfidf.txt"
        textrank_path = "se2010_res/" + name + "textrank.txt"
        rake_path = "se2010_res/" + name + "rake.txt"
